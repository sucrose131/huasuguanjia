...
async function load() {
  ...
  try {
    ...
    const data = (await api.get('/goods/categories', { params })) as any;
...
<template>
  <section class="page">
    <header class="page-head">
      <div>
        <h2>商品分类</h2>
        <p class="page-subtitle">通过上级分类表达层级，并维护仓库类型和继承属性</p>
      </div>
    ...
    <div class="panel">
      ...
      <div class="query-bar">
        <el-input
          ...
          placeholder="分类名称"
          @keyup.enter="search"
        /><el-select v-model="query.parentId" class="query-field" clearable placeholder="上级分类"
          ><el-option
      ...
      <div v-else class="table-wrap">
        <el-table :data="rows" v-loading="loading"
          ...
          /><el-table-column label="上级分类" min-width="140"
            ><template #default="s">{{ s.row.parentName || '顶级分类' }}</template></el-table-column
          ><el-table-column label="仓库类型" width="110"
            ><template #default="s">{{
              dict(warehouseTypes, s.row.warehouseType)
            }}</template></el-table-column
          ><el-table-column label="关联属性" min-width="190" show-overflow-tooltip
            ><template #default="s">{{
              s.row.propertyNames?.join('、') || '—'
            }}</template></el-table-column
          ><el-table-column prop="propertyCount" label="属性数" width="80" /><el-table-column
            prop="goodsCount"
            label="商品数"
            width="80"
          /><el-table-column prop="childCount" label="子分类数" width="90" /><el-table-column
            prop="sort"
            label="排序"
            width="75"
      ...
      <footer v-if="rows.length" class="table-footer">
        <span class="result-total">共 {{ total }} 条商品分类</span
        ><el-pagination
    ...
    <el-dialog
      ...
      ><template v-if="false"
        ><div class="detail-grid">
          ...
          <div class="detail-item">
            <span class="detail-label">上级分类</span
            ><span>{{ form.parentName || '顶级分类' }}</span>
          </div>
          <div class="detail-item">
      ...
      ><el-form v-else label-position="top" :disabled="mode === 'view'"
        ><div class="dialog-grid">
          ...
          ><el-form-item label="上级分类"
            ><el-select v-model="form.parentId" style="width: 100%"
              ><el-option label="顶级分类" :value="0" /><el-option
                v-for="item in categories.filter((c) => String(c.id) !== String(editing))"
                :key="item.id"
                :label="item.name"
          ...
          ><el-form-item label="排序"
            ><el-input-number v-model="form.sort" :min="0" style="width: 100%" /></el-form-item
