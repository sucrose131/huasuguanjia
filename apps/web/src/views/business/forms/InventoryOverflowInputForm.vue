...
const dicts = reactive<Record<string, any[]>>({ overflowType: [] });
...
const documentTypeName = computed(
  () =>
    ...
    (dicts.overflowType ?? []).find(
      (x) => String(x.value) === String(form.value.documentType),
    )?.label ??
    '—',
);
const inputNo = computed(() => form.value.inputNo || form.value.businessNo || '—');
const inputTime = computed(() => dateText(form.value.inputDate, true) || '—');
...
onMounted(async () => {
  const [orgs, warehouses, departments, overflowType] = await Promise.all([
  ...
  dicts.overflowType = overflowType as any[];
  if (form.value.id) {
  ...
  <el-form label-position="top" :disabled="true">
    <div class="master-grid">
      <el-form-item label="入库单号">
        <el-input :model-value="inputNo" disabled />
      </el-form-item>
      <el-form-item label="来源报盈单">
      ...
      <el-form-item label="确认入库时间">
        <el-input :model-value="inputTime" disabled />
      </el-form-item>
    ...
    <div class="details-header">
      <span class="details-title">报盈明细</span>
