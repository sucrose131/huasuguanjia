...
const moneyLimit = computed(() =>
  Math.max(0, Number(form.value.unreceivedAmount ?? form.value.availableAmount ?? 0) || 0),
);
...
function validate() {
  ...
  if (Number(form.value.amount) > moneyLimit.value + 0.000001) {
    ElMessage.warning('本次收款金额不能超过未收金额');
    return false;
  }
  return true;
}
...
async function save() {
  ...
  saving.value = true;
  try {
    await ElMessageBox.confirm('收款保存后立即生效且不可直接编辑，是否继续？', '确认', {
      type: 'warning',
    });
    ...
    const result: any = await api.post('/sales/payments', payload);
    ElMessage.success(result?.message ?? '收款成功');
    emit('saved');
  } catch {
    // axios 拦截器已提示
  } finally {
...
onMounted(async () => {
  ...
  if (props.mode === 'create') {
    ...
    const detail: any = await api.get(`/sales/payments/${form.value.id}`).catch(() => null);
...
<template>
  <el-form label-position="top" :disabled="isView">
    ...
    <div class="form-grid">
      <el-form-item label="本次收款金额" required class="span-2">
        <el-input-number
          v-model="form.amount"
      ...
      <el-form-item label="备注" class="span-2">
        <el-input v-model="form.remark" type="textarea" :rows="2" placeholder="可填写收款说明" />
      </el-form-item>
    </div>
