...
const orderTotals = computed(() => {
  ...
  const quantity = details.reduce((s: number, x: any) => s + Number(x.quantity || 0), 0);
  const orderAmount = details.reduce(
  ...
  const factAmount = details.reduce((s: number, x: any) => s + lineActual(x), 0);
  return { quantity, orderAmount, discount, factAmount };
});

async function loadDicts() {
  const codes = ['sales_order_source', 'sales_order_property', 'sales_business_source_type'];
...
async function save() {
  ...
  saving.value = true;
  try {
    const payload: Record<string, any> = { ...form.value, propertyType: 2 };
    const url = '/sales/discount-orders';
    const result: any =
      props.mode === 'edit'
...
onMounted(async () => {
  if (props.mode !== 'view' && !canEditAmount.value) {
...
<template>
  <el-form label-position="top" :disabled="isView">
    <div class="form-grid">
      ...
      <el-form-item label="仓库" required>
        <el-select
          ...
          filterable
          :disabled="isView || !form.orgId || sourceLocked"
      ...
      <el-form-item label="来源类型">
        <el-select v-model="form.businessSourceType" disabled>
          <el-option
            v-for="x in dicts.sales_business_source_type || []"
            :key="x.value"
            :label="x.label"
      ...
      <el-form-item label="订单来源">
        <el-select v-model="form.sourceType" :disabled="isView || sourceLocked">
          <el-option
    ...
    <div class="details-header">
      ...
      <el-button v-if="!isView && !sourceLocked" link type="primary" @click="addLine">
        + 添加商品
      </el-button>
    </div>
    <el-table :data="form.details ?? []" border size="small">
      ...
      <el-table-column label="最终销售单价" width="140">
        <template #default="s">
          <el-input-number
            v-model="s.row.price"
            :min="0"
      ...
      <el-table-column label="数量" width="130">
        <template #default="s">
          <el-input-number
            ...
            :step="1"
            :disabled="isView || sourceLocked"
            @change="refreshAvailableStock(s.row)"
          />
        </template>
      </el-table-column>
      <el-table-column label="旧物折价金额" width="150">
        <template #default="s">
          <el-input-number
            v-if="!isView"
            v-model="s.row.factAmount"
            :min="0"
            :precision="2"
            :controls="false"
      ...
      <el-table-column label="备注" min-width="140">
        <template #default="s">
          <el-input v-model="s.row.remark" :disabled="isView || sourceLocked" />
        </template>
      </el-table-column>
      <el-table-column v-if="!isView && !sourceLocked" label="" width="60">
        <template #default="s">
