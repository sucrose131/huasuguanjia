...
async function loadWarehouseTabs() {
  warehouseTabs.value = (await api
    .get('/inventory/warehouses/tabs')
    .catch(() => [])) as any[];
}

function selectWarehouse(query: Record<string, any>, load: () => void, value: unknown) {
  ...
  <BusinessDocumentPage ref="pageRef" :config="inventoryCheckConfig">
    ...
    <template #query-tools="{ query, load, total }">
      <div class="warehouse-tabs">
        <button :class="{ active: !query.warehouseId }" @click="selectWarehouse(query, load, '')">
          全部 <span>{{ total }}</span>
        </button>
        <button
          ...
          @click="selectWarehouse(query, load, item.value)"
        >
