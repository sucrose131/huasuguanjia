...
async function main() {
  const checks = {
    ...
    linkedUsersWithBrokenIdentityChain: await query(`
      SELECT user.id user_id, user.username, user.staff_id, user.org_id, user.dept_id,
             membership.org_type, membership.org_id primary_membership_id,
      ...
      WHERE user.deleted_at IS NULL AND user.staff_id IS NOT NULL AND (
        staff.id IS NULL OR membership.id IS NULL OR
        (membership.org_type = 1 AND user.org_id <> membership.org_id) OR
        (membership.org_type = 2 AND (user.org_id <> dept.org_id OR user.dept_id <> dept.dept_id))
      )
    `),
    invalidAuthorizedOrganizations: await query(`
      ...
      WHERE (user.id IS NOT NULL AND user.status = 1 AND org.org_id IS NULL)
    `),
  ...
  const warnings = {
    staffWithoutActivePosition: await query(`
      SELECT staff.id staff_id, staff.name, staff.post_id
      FROM hspsi_basic_staff staff
      ...
      WHERE staff.deleted_at IS NULL AND staff.status = 1 AND position.id IS NULL
    `),
    positionsWithoutMembershipMapping: await query(`
      SELECT DISTINCT staff.post_id position_id, position.name position_name, staff.account_set_id
      FROM hspsi_basic_staff staff
      JOIN hspsi_basic_position position
        ON position.id = staff.post_id AND position.deleted_at IS NULL AND position.status = 1
      JOIN hspsi_basic_staff_organizations membership
        ON membership.staff_id = staff.id AND membership.type = 1 AND membership.deleted_at IS NULL
      WHERE staff.deleted_at IS NULL AND staff.status = 1
        AND NOT EXISTS (
          SELECT 1 FROM hspsi_basic_position_belongs belongs
          WHERE belongs.position_id = staff.post_id
            AND belongs.account_set_id = staff.account_set_id
        )
    `),
    disabledUsersWithRetainedScope: await query(`
      SELECT DISTINCT user.id user_id, user.username
      FROM hspsi_sys_user user
      JOIN hspsi_sys_user_authorized_org scope ON scope.user_id = user.id
      WHERE user.deleted_at IS NULL AND user.status <> 1
    `),
  };
  ...
  const warningOutput = Object.fromEntries(
    Object.entries(warnings).map(([name, rows]) => [name, { count: rows.length, rows }]),
  );
  const issueCount = Object.values(checks).reduce((sum, rows) => sum + rows.length, 0);
  const warningCount = Object.values(warnings).reduce((sum, rows) => sum + rows.length, 0);
  console.log(
    JSON.stringify(
      { generatedAt: new Date().toISOString(), issueCount, warningCount, checks: output, warnings: warningOutput },
