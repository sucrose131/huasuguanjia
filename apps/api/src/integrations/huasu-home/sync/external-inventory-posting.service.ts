...
import type { InventoryLine, InventoryPosting } from '../../../inventory/inventory-posting.service';
 ...
 */
@Injectable()
export class HuasuHomeExternalInventoryPostingService {
  ...
  private async postInTransaction(tx: Prisma.TransactionClient, input: InventoryPosting) {
    ...
    for (const line of input.lines as InventoryLine[]) {
      const goodsId = BigInt(String(line.goodsId));
      const skuId = BigInt(String(line.skuId));
      ...
      // 允许负库存：无明细则后续 upsert create，有明细也不校验是否够扣
