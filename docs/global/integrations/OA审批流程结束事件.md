...
华溯管家订阅的是应用级 `XFTOAFPS`，会收到该应用下全部表单的终态通知。解密后的 `procKey` 实测格式为 `FORM_` + 表单 `form_key`，例如：
...
仅当还原后的 `form_key` 存在于当前账套的 `hspsi_oa_form_template`（`status=1`、未删除）时，才进入领用/采购/库存/生产/销售审批结果处理；其余表单成功回包并记「非本系统表单，已忽略」。