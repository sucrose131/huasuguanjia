...
> - 正式事件按本文对 `eventId`、`prjCod`、`eventTime`、`eventCd` 做 SM2 验签，并用公钥前 32 位 hex 对 `eventRcdInf` 做 SM4 解密。
> - 验签公钥配置 `XINFUTONG_OA_EVENT_PUBLIC_KEY`（配置回调 URL 后由薪福通生成，130 位 hex）。
