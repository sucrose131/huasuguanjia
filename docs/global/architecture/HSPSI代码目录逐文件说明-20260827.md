...
- `purchase-application.ts`：采购申请列表、提交、撤回、终止、删除和生成订单配置。
...
- `purchase-oa-approval.service.spec.ts`：测试采购申请 OA 审批发起与主动撤销。
- `purchase-oa-approval.service.ts`：提交采购申请到 OA，以及创建人主动撤销运行中的 OA 流程。
...
- `purchase.controller.ts`：采购申请、订单、入库、退货、付款和退款 API，含创建人终止和撤回审批。
- `purchase.module.ts`：注册采购模块。
- `purchase.service.spec.ts`：测试采购完整链路。
- `purchase.service.ts`：采购申请、拆单、订单、入库、退货、付款、退款及创建人终止/撤回审批逻辑。
...
- `approval.service.ts`：调用 OA 审批接口（发起流程、流程处理、文件上传），管理审批实例标识和提交结果。
- `approval.types.ts`：定义 OA 审批提交、流程处理、实例状态和回调数据类型。
...
该目录保存薪福通 OA 专属接口资料。事件订阅对应后端 `xinfutong-oa/approval/event-envelope.ts` 和回调服务；流程处理对应 `xinfutong-oa/approval/approval.service.ts` 的 `dealProcess`。

- `事件订阅概述.md`：OA 事件订阅、回调、验签和重试概述。
- `审批流程处理.md`：流程处理接口（提交、通过、否决、转派、加签、退回、撤销）的请求、响应和错误码。
