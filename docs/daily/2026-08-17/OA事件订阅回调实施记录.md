...
薪福通 OA 审批结果通过事件订阅回传华溯管家。回调地址必须是通用事件入口，不能写死某个事件编号；接收后按官方事件订阅协议验签、解密，并在 3 秒内按约定报文体返回。验签公钥按应用配置，不能使用全局环境变量。
...
- 正式事件对 `eventId`、`prjCod`、`eventTime`、`eventCd` 做 SM2 验签；`eventRcdInf` 使用该应用公钥前 32 位 hex 做 SM4 解密。
- 验签公钥按应用保存在 `hspsi_sys_account_set.event_public_key`，用事件报文 `appId` 匹配 `app_id`。已从环境变量示例中移除 `XINFUTONG_OA_EVENT_PUBLIC_KEY`。
...
| 无页面 | 应用 ID | appId | hspsi_sys_account_set | app_id | 正式事件必填 | 仅用于匹配应用 |
...
| 无页面 | 应用事件验签公钥 | — | hspsi_sys_account_set | event_public_key | 正式事件必填，默认空 | 按应用配置，不走环境变量 |
...
| --- | --- |
| 连接测试事件回包 | 自动化通过 |
| 按 appId 取应用公钥后 SM2 验签 + SM4 解密 | 自动化通过 |
| 未知 appId 拒绝验签 | 自动化通过 |
...
部署后需在薪福通开发者后台把事件订阅 URL 改为统一入口，并把该应用生成的接口鉴权公钥写入对应账套的 `hspsi_sys_account_set.event_public_key`。未配置公钥时，连接测试可通过，正式事件会验签失败。

## 六、遗留问题

- 账套维护目前没有页面，公钥需直接写入数据库。
